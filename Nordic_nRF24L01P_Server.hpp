...
// MapOS RPC server object.
template <typename MIRF_t>
class Nordic_nRF24L01P_Server : public SimpleServer, public Process {
// mirf is stored by value, not reference.
  MIRF_t mirf;
...
public:
  Nordic_nRF24L01P_Server(MIRF_t new_mirf, MemoryPool *new_memoryPool, MAP::MAPPacketSink *new_outputPacketSink)
...
private:
  bool replyReceivedPacket(ReceivedPacket &rPacket);
};


template <typename MIRF_t>
bool Nordic_nRF24L01P_Server<MIRF_t>::replyReceivedPacket(ReceivedPacket &rPacket){
...
template <typename MIRF_t>
Status::Status_t Nordic_nRF24L01P_Server<MIRF_t>::process(){
  // Anything to process?
...
// Opcode processing.
  ...
  /* CONFIGURATION CONTROL */
    case 0: { // set_mode(enum)
      ...
      DEBUGprint("n-sM:%d;", raw_value? Mode_TX : Mode_RX);
      mirf.set_mode(raw_value? Mode_TX : Mode_RX);
  ...
  /* RF CONTROL */
    ...
    OPCODE(19) // set_data_rate(Rate_t new_rate)  <-- enum
      GET_RAW_UINT(raw_value);
      Rate_t new_rate;
      if(raw_value == 250) new_rate = Rate_250K;
      else if(raw_value == 1000) new_rate = Rate_1M;
      else if(raw_value == 2000) new_rate = Rate_2M;
      else break;
